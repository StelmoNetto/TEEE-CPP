    ...
    inline namespace SobrecargaDeOperadores
    {
      inline namespace Binario
      {
        inline namespace ComMetodo
        {
          ...
          const UmaClasse UmaClasse::operator+(const UmaClasse& outroObjeto) const
          {
              UmaClasse resultado{ *this->membroAlocado + *outroObjeto.membroAlocado };

              return resultado;
          }

          const UmaClasse UmaClasse::operator+ (int valor) const
          {
              return UmaClasse{ *this->membroAlocado + valor };
          }
          ...
          int UmaClasse::obterDado() const { return (membroAlocado) ? *membroAlocado : 01; }
